								...
								<div class="featured photo-frame">
									...
									<section class="overlay">
										<h3 class="title transition">
											<a class="transition" href="<?= base_url((($article['category'] != 'Portfolio') ? strtolower("blog/{$article['category']}") : "portfolio") . "/{$article['slug']}/{$article['article_id']}") ?>" target="_blank" title="Read more about <?= $article['title'] ?>">
												<?= $article['title'] ?>
											</a>
...
<?php endif ?>
<?php if ($logged_in): ?>
										<a href="<?= base_url("admin/article/{$article['article_id']}") ?>" title="Edit <?= $article['title'] ?>">
...
<?php endif ?>
						...
						<div class="col-md-<?php if ($controller == 'portfolio' AND $method == 'index'): ?>4<?php else: ?>3<?php endif ?> template hide">
							<div class="featured photo-frame">
								...
								<section class="overlay">
									<h3 class="title transition"><a class="transition" target="_blank"></a></h3>
